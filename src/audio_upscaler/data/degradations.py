...
Generates paired (degraded, clean) examples on-the-fly using ffmpeg.
Supports MP3, AAC, Opus codecs at multiple bitrates, plus resampling,
low-pass filtering, stereo collapse, and mild clipping.
"""
...
class Degrader:
    """Apply random degradations to a waveform.

    Args:
        sample_rate:          target sample rate
        mp3_bitrates:         list of MP3 bitrates (kbps)
        aac_bitrates:         list of AAC bitrates (kbps)
        opus_bitrates:        list of Opus bitrates (kbps)
        resample_rates:       intermediate rates for down/up resampling
        prob_lowpass:         probability of adding a lowpass filter
        lowpass_cutoff_hz:    lowpass cutoff before codec
        prob_stereo_collapse: probability of collapsing stereo to mono and back
        prob_clipping:        probability of soft clipping
        clipping_threshold:   normalised clipping threshold (0..1)
    """
    ...
    def degrade(self, waveform: torch.Tensor) -> torch.Tensor:
        """Apply a random degradation and return the degraded waveform.

        Args:
            waveform: (C, T) float32 tensor, range [-1, 1]

        Returns:
            degraded: (C, T) float32 tensor, same shape
        """
        audio_np = waveform.numpy().T  # (T, C) for soundfile

        # Optional: stereo collapse
        if audio_np.shape[1] > 1 and random.random() < self.prob_stereo_collapse:
            mono = audio_np.mean(axis=1, keepdims=True)
            audio_np = np.concatenate([mono, mono], axis=1)

        # Optional: low-pass filter via ffmpeg
        apply_lowpass = random.random() < self.prob_lowpass

        # Codec degradation
        codec = random.choice(self.CODEC_CHOICES)
        audio_np = self._codec_degrade(audio_np, codec, apply_lowpass)

        # Optional: resample down/up
        if random.random() < 0.3:
            target_sr = random.choice(self.resample_rates)
            audio_np = self._resample(audio_np, target_sr)
        ...
        if random.random() < self.prob_clipping:
            ...
            audio_np /= self.clipping_threshold  # renormalise
    ...
    def _build_ffmpeg_filters(
        self, codec: str, bitrate: int, apply_lowpass: bool
    ) -> tuple[list[str], list[str]]:
        """Return (input_filters, output_codec_args) for ffmpeg."""
        in_filters: list[str] = []
        if apply_lowpass:
    ...
    def _codec_degrade(self, audio_np: np.ndarray, codec: str, apply_lowpass: bool) -> np.ndarray:
        bitrates = {
        ...
        in_filters, codec_args, ext = self._build_ffmpeg_filters(codec, bitrate, apply_lowpass)
    ...
    def _resample(self, audio_np: np.ndarray, target_sr: int) -> np.ndarray:
        """Resample down to target_sr then back to self.sample_rate via ffmpeg."""
        n_channels = audio_np.shape[1]
