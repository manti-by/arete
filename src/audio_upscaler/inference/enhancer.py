"""Inference engine with overlap-add chunked processing."""
...
class Enhancer:
    """Load a trained model and enhance audio files.

    Args:
        model:          trained nn.Module
        sample_rate:    model's target sample rate
        chunk_seconds:  processing window size
        hop_seconds:    overlap-add hop (< chunk_seconds for overlap)
    ...
    def enhance_waveform(self, waveform: torch.Tensor) -> torch.Tensor:
        """Enhance a waveform tensor with overlap-add.
        ...
        while start < n_t:
            ...
            chunk_device = chunk.unsqueeze(0).to(self.device)  # (1, C, T)
            pred = self.model(chunk_device).squeeze(0).cpu()  # (C, T)
    ...
    def enhance_file(self, input_path: str | Path, output_path: str | Path) -> None:
        """Load, enhance, and save an audio file."""
